"""Initial schema — well telemetry, anomaly alerts, RAG store, investigations,
escalation queue, and agent audit log.

Mirrors infra/sql/init.sql so that both the Docker bootstrap path (init.sql
via docker-entrypoint-initdb.d) and the migration path (alembic upgrade head)
produce an identical schema. All CREATE TABLE / INDEX statements use
IF NOT EXISTS so either path is idempotent.

Revision ID: a1b2c3d4e5f6
Revises:
Create Date: 2025-01-15 09:00:00.000000
"""
...
def upgrade() -> None:
    ...
    """)
    op.execute(
        "CREATE INDEX IF NOT EXISTS idx_telemetry_well_ts "
        "ON well_telemetry (well_id, timestamp DESC)"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS idx_telemetry_field "
        "ON well_telemetry (field_name)"
    )
    ...
    """)
    op.execute(
        "CREATE INDEX IF NOT EXISTS idx_alerts_well "
        "ON anomaly_alerts (well_id, timestamp DESC)"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS idx_alerts_severity "
        "ON anomaly_alerts (severity, investigated)"
    )
    ...
    """)
    op.execute(
        "CREATE INDEX IF NOT EXISTS idx_investigations_well "
        "ON investigations (well_id, timestamp DESC)"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS idx_investigations_escalate "
        "ON investigations (should_escalate, created_at DESC)"
    )
    ...
    op.execute(
        "CREATE INDEX IF NOT EXISTS idx_escalations_well "
        "ON escalations (well_id, status)"
    )
    ...
    op.execute(
        ...
        "ON agent_audit_log (investigation_id)"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS idx_audit_agent "
        "ON agent_audit_log (agent_id, timestamp DESC)"
    )
...
def downgrade() -> None:
    ...
    op.execute("DROP TABLE IF EXISTS well_telemetry CASCADE")
